...
def log_sensors():
    ...
    now = datetime.now()
    humidity = int(
        bytes(sensor_read["humidity"])[0]
    )  # int.from_bytes(value_1, "little")
    temperature = struct.unpack("f", sensor_read["temperature"])[0]
    pressure = struct.unpack("f", sensor_read["pressure"])[0]
    co2 = int(bytes(sensor_read["co2"])[0])  # int.from_bytes(value_4, "little")
    iaq = struct.unpack("f", sensor_read["iaq"])[0]
    gasresistance = int(
        bytes(sensor_read["gasresistance"])[0]
    )  # value_6 = int.from_bytes(value_6, "little")
    accuracy = int(
        bytes(sensor_read["accuracy"])[0]
    )  # value_7 = int.from_bytes(value_7, "little")

    str_to_write = f"{str(now)},{str(temperature)},{str(humidity)},{str(pressure)},{str(gasresistance)},{str(co2)},{str(iaq)},{str(accuracy)}\n"
...
def main():
    ...
    with open(filepath_out, "w") as fp:
        fp.write(
            "datetime,temperature,humidity,pressure,gasresistance,co2,iaq,accuracy\n"
        )
